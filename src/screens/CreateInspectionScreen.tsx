...
export default function CreateInspectionScreen() {
  ...
  const saveInspection = async () => {
    ...
    // Проверяем, что все нарушения имеют описание
    const violationsWithDescription = violations.filter(v => v.description.trim());
    if (violationsWithDescription.length === 0) {
      Alert.alert('Внимание', 'Заполните описание хотя бы одного нарушения');
      return;
    }
    ...
    try {
      // Сохраняем нарушения
      for (const violation of violationsWithDescription) {
      ...
      const inspectionReport = await ReportService.generateInspectionAct(objectId, {
        violations: violationsWithDescription,
        photos,
        comments,
        recommendations,
        inspector: user?.fullName || 'Инспектор',
